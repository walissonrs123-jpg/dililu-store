...
- Trust exata em [iam/dililu-trust.json](iam/dililu-trust.json): somente `repo:walissonrs123-jpg@240164307/dililu-store@1387345561:ref:refs/heads/main`, sem environment no job.
- Policy inline `dililu-publish` em [iam/dililu-publish.json](iam/dililu-publish.json): somente `s3:ListBucket` no bucket Dililu, `s3:PutObject` nos objetos e `cloudfront:CreateInvalidation`/`cloudfront:GetInvalidation` somente na distribuição `E1EZ9JK9Q7UT7V`. Sem outras policies anexadas.
...
- `.github/workflows/deploy.yml`: somente disparo manual na main, build/export, autenticação OIDC, PutObject no bucket Dililu, invalidação exclusiva com espera e testes de navegador; permissões `contents: read` e `id-token: write`. Action fixada no SHA oficial da v5.1.1. Nenhum segredo permanente no GitHub.
- Autenticação ponta a ponta PASS na tentativa 2 do [workflow 36133208998](https://github.com/walissonrs123-jpg/dililu-store/actions/runs/36133208998). A primeira tentativa revelou subject imutável do GitHub; somente o subject da trust foi corrigido com IDs confirmados pela API, preservando repositório/main e permissões. `sts get-caller-identity` e comparação da role passaram na única repetição autorizada. Publicador autorizado e validado posteriormente no workflow 36167487484; ativação DNS posteriormente autorizada e concluída; evidências em AWS_VALIDATION.md.
...
- Atualização autorizada: adicionado exclusivamente `cloudfront:GetInvalidation` à inline `dililu-publish`; leitura antes/depois confirmou preservação das três permissões anteriores. Trust intocada; sem ListInvalidations, publicação ou alterações S3/DNS/Terraform.
...
## Publicação estática autorizada
- Publicador scripts/publish-static.py envia somente out/, exclui .gitkeep, bloqueia caminhos privados e symlinks, preserva objetos existentes e serve aliases HTML sem alteração de infraestrutura. MIME explícito e cache imutável somente em _next/static/.
- scripts/smoke-cloudfront.mjs validou Chromium desktop/mobile na URL CloudFront; WhatsApp interceptado no teste, sem envio de mensagem. Nenhum erro de console/runtime nos fluxos testados.
- Referências: [Next.js export](https://nextjs.org/docs/app/guides/static-exports), [Playwright CI](https://playwright.dev/docs/ci-intro).
